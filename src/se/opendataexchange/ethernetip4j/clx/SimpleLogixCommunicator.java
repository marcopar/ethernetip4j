...
public class SimpleLogixCommunicator extends ControlLogixConnector {
    ...
    String convertToString(Object[] oo) {
        char cc[] = new char[oo.length];
        for(int i = 0; i < cc.length; i++) {
            cc[i] = ((Character) oo[i]).charValue();
        }
        String s = new String(cc);
        return s;
    }

    Object[] convertToArray(String s) {
        Object[] oo = new Object[s.length()];
        for(int i = 0; i < oo.length; i++) {
            oo[i] = new Character(s.charAt(i));
        }
        return oo;
    }

    public String readString(String tagName) throws PathSegmentException,
        ItemNotFoundException, ProcessingAttributesException,
        InsufficientCommandException, InsufficientNrOfAttributesException,
        OtherWithExtendedCodeException, ResponseBufferOverflowException,
        InvalidTypeException, IOException, EmbeddedServiceException, NotImplementedException {
        return readString(tagName, (byte) 1, (byte) 0);
    }

    public String readString(String tagName, byte ethernetSlot, byte cpuSlot) throws PathSegmentException,
        ItemNotFoundException, ProcessingAttributesException,
        InsufficientCommandException, InsufficientNrOfAttributesException,
        OtherWithExtendedCodeException, ResponseBufferOverflowException,
        InvalidTypeException, IOException, EmbeddedServiceException, NotImplementedException {

        String tagNameLen = String.format("%s.LEN", tagName);
        String tagNameData = String.format("%s.DATA", tagName);

        int len = (Integer) read(tagNameLen, ethernetSlot, cpuSlot);
        if(len == 0) {
            return "";
        }
        return convertToString((Object[]) read(tagNameData, len, ethernetSlot, cpuSlot));
    }

    public void writeString(String tagName, String value) throws PathSegmentException,
        ItemNotFoundException, ProcessingAttributesException,
        InsufficientCommandException, InsufficientNrOfAttributesException,
        OtherWithExtendedCodeException, ResponseBufferOverflowException,
        InvalidTypeException, IOException, EmbeddedServiceException, NotImplementedException {

        writeString(tagName, value, (byte) 1, (byte) 0);
    }

    public void writeString(String tagName, String value, byte ethernetSlot, byte cpuSlot) throws PathSegmentException,
        ItemNotFoundException, ProcessingAttributesException,
        InsufficientCommandException, InsufficientNrOfAttributesException,
        OtherWithExtendedCodeException, ResponseBufferOverflowException,
        InvalidTypeException, IOException, EmbeddedServiceException, NotImplementedException {
        ...
        if(len != 0) {
            Object oo[] = convertToArray(value);
            write(tagNameData, oo, len, ethernetSlot, cpuSlot);
        }
        write(tagNameLen, len, 1, ethernetSlot, cpuSlot);
    }
}
