...
public class UnconnectedMessageManager {
    ...
    public ArrayList<EthernetIpPacket> checkForData(String host, ArrayList<String> tagList) {
        synchronized(unitConnections) {
            ...
            /*
             * Set set = unitConnections.entrySet(); printObject(CONNECTOR.read("rdTag1")); printObject(CONNECTOR.read("tag")); printObject(CONNECTOR.read("tag1234"));
             * printObject(CONNECTOR.read("float1")); printObject(CONNECTOR.read("test")); for (Object obj : set) { ((UnconnectedConnection)((Entry)obj).getValue()).checkAndProcessData();
			}
             */
        }
