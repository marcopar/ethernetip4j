...
public class UnconnectedMessageManagerRequestTest {
    ...
    public void testGetWriteRequestBuffer() throws InvalidTypeException, NotImplementedException {
        ...
        //Assert.assertEquals(expected.length, actual.length);
		/*
         * for (int i=0; i<Math.min(actual.length, expected.length); i++){ if (i < actual.length && i < expected.length) Log.p(i+"\t" +HexConverter.byte2hex(actual[i]) +" "
         * +HexConverter.byte2hex(expected[i])); else if (i < actual.length) Log.p(i+"\t" +HexConverter.byte2hex(actual[i]) +" xx"); else if (i < expected.length) Log.p(i+"\t xx "
         * +HexConverter.byte2hex(expected[i]));
		}
         */
