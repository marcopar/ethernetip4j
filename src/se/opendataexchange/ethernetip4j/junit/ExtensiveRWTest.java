...
public class ExtensiveRWTest {
    ...
    /*
     * TODO: @org.junit.Test public void testArrayBOOL() throws IOException, PathSegmentException, ItemNotFoundException, ProcessingAttributesException, InsufficientCommandException,
     * InsufficientNrOfAttributesException, OtherWithExtendedCodeException, ResponseBufferOverflowException, InvalidTypeException, EmbeddedServiceException, NotImplementedException{
     * extensiveRWArr("ArrayBOOL", new Float[5000]);
	}
     */
    /*
     * @org.junit.Test public void testArrayBOOL() throws IOException, PathSegmentException, ItemNotFoundException, ProcessingAttributesException, InsufficientCommandException,
     * InsufficientNrOfAttributesException, OtherWithExtendedCodeException, ResponseBufferOverflowException, InvalidTypeException, EmbeddedServiceException, NotImplementedException{ Boolean[] a = new
     * Boolean[1]; Boolean[] b = new Boolean[500]; Boolean[] c = new Boolean[4500];
     *
     * for (int i=0; i<a.length; i++){ a[i] = i%2==0; } for (int i=1; i<b.length; i++){ b[i] = i%3==0; } for (int i=1; i<c.length; i++){ c[i] = i%5!=0; } //Boolean[][] x = new Boolean[][]{a, b, c};
     * //extensiveRWArr("ArrayBool", x); connector.write("ArrayBool", a, a.length); Object o = connector.read("ArrayBool", a.length);
     *
     * if (o instanceof Object[]){ Object[] oArr = (Object[])o; System.out.println("1. Class: "+oArr.getClass().toString()); for (Object item : oArr){ System.out.println("2. Class:
     * "+item.getClass().toString()); if (item instanceof Object[]){ System.out.println("Is array!"); }else if (item instanceof byte[]){ System.out.println("is byte array "+((byte[])item).length);
     * byte[] bitem = (byte[])item; for(byte bi : bitem){ System.out.println(bi +", "); } } } } }
     */
    /**
     * *
